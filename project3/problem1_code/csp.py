...
class CSP(search.Problem):
    ...
    def __init__(self, variables, domains, neighbors, constraints):
        ...
        self.nassigns = 0
        # for weights define a dictionary with the constraint as key and the weight as value
        self.constraint_weights = {}  # Initialize weights for all constraints
        self.constraint_weights.update({(var, neighbor): 1 for var in variables for neighbor in neighbors[var]})
...
def dom_wdeg_ratio(var, assignment, csp):
    ...
    # if we have no neighbors (sum == 0) that means the only unassigned variable is the 
    # only one unassigned left so it doesnt even matter what we return. just set the sum to 1
    if sum == 0: 
        sum = 1 
    # else we update the min_var if we have Found
    # a better ratio dom / wdeg
    if not csp.curr_domains: # if we have no curr_domains that means we didnt prune anything from var yet so we use the normal domains
